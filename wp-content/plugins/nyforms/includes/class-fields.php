...
class Fields {
	...
	public static function sanitize( $field ) {
		...
		$size = absint( $field['size'] ?? 100 ); if ( ! in_array( $size, array( 25, 50, 75, 100 ), true ) ) { $size = 100; }
		$clean = array( 'key' => $key, 'type' => $type, 'label' => sanitize_text_field( $field['label'] ?? '' ), 'description' => wp_kses_post( $field['description'] ?? '' ), 'placeholder' => sanitize_text_field( $field['placeholder'] ?? '' ), 'help' => wp_kses_post( $field['help'] ?? '' ), 'default' => sanitize_text_field( $field['default'] ?? '' ), 'css_class' => sanitize_html_class( $field['css_class'] ?? '' ), 'size' => $size, 'required' => ! empty( $field['required'] ), 'choices' => $choices, 'visibility' => Conditions::sanitize( $field['visibility'] ?? array() ), 'max_files' => min( 10, max( 1, absint( $field['max_files'] ?? 1 ) ) ), 'max_size' => min( 50 * MB_IN_BYTES, max( 1, absint( $field['max_size'] ?? wp_max_upload_size() ) ) ), 'allowed_types' => sanitize_text_field( $field['allowed_types'] ?? '' ), 'price' => (float) ( $field['price'] ?? 0 ), 'formula' => sanitize_text_field( $field['formula'] ?? '' ) );
		$provider = Extensions::field_type( $type ); return $provider ? $provider->sanitize( $clean ) : $clean;
	}
	public static function validate( $field, $value ) {
		...
		$provider = Extensions::field_type( $field['type'] ); if ( $provider ) { return $provider->validate( $value, $field ); }
