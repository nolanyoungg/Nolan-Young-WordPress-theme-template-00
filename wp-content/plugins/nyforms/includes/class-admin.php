...
class Admin {
	...
	public function help_page() {
		...
		echo '<section class="nyforms-help-card"><h2>' . esc_html__( 'Manage entries', 'nyforms' ) . '</h2><p>' . esc_html__( 'Entries provides form-specific search, unread and starred views, spam and trash states, pagination, and bulk actions.', 'nyforms' ) . '</p><p>' . esc_html__( 'Export active submissions as CSV or Excel-compatible XLS from Import/Export, or use Entries for a direct CSV export.', 'nyforms' ) . '</p></section>';
		echo '<section class="nyforms-help-card"><h2>' . esc_html__( 'Import and export', 'nyforms' ) . '</h2><p>' . esc_html__( 'Import a versioned NYforms JSON definition to create a new form. Export JSON to move a form definition, fields, and settings between WordPress sites.', 'nyforms' ) . '</p><p>' . esc_html__( 'JSON form exports do not include submissions. CSV and XLS exports contain active entries for the selected form.', 'nyforms' ) . '</p></section>';
		echo '<section class="nyforms-help-card"><h2>' . esc_html__( 'Privacy and protection', 'nyforms' ) . '</h2><p>' . esc_html__( 'Submissions receive server-side validation and nonce checks. Forms can use a honeypot, and site-wide rate limiting is configured in Settings.', 'nyforms' ) . '</p><p>' . esc_html__( 'Configure entry retention and uninstall cleanup in Settings. NYforms also integrates with WordPress personal-data export and erasure tools.', 'nyforms' ) . '</p></section>';
		echo '<section class="nyforms-help-card"><h2>' . esc_html__( 'Current scope', 'nyforms' ) . '</h2><p>' . esc_html__( 'NYforms does not currently provide payment gateways, bundled CAPTCHA service keys, automatic third-party data transmission, Gravity Forms imports, or copied third-party interfaces.', 'nyforms' ) . '</p><p>' . esc_html__( 'Add-On providers can extend fields, anti-spam checks, and notifications when registered by a site owner or developer.', 'nyforms' ) . '</p></section>';
		echo '<section class="nyforms-help-card"><h2>' . esc_html__( 'Troubleshooting', 'nyforms' ) . '</h2><p>' . esc_html__( 'Check System Report for upload permissions, database tables, REST availability, scheduled tasks, and server details. Make sure the form is active before testing it on the site.', 'nyforms' ) . '</p><p><a class="button" href="' . esc_url( admin_url( 'admin.php?page=nyforms-system-status' ) ) . '">' . esc_html__( 'Open System Report', 'nyforms' ) . '</a> <a class="button" href="' . esc_url( admin_url( 'admin.php?page=nyforms-settings' ) ) . '">' . esc_html__( 'Open Settings', 'nyforms' ) . '</a></p></section>';
		echo '</div></div>';
	}
