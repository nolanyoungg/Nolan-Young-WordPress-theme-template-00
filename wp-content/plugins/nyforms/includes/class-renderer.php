...
class Renderer {
	public function render( $form_id, $args = array() ) {
		...
		$settings = get_option( 'nyforms_settings', array() ); $recaptcha_type = $settings['recaptcha_type'] ?? 'checkbox'; $recaptcha_enabled = ! empty( $settings['recaptcha_enabled'] ) && ! empty( $settings['recaptcha_site_key'] ) && ! empty( $settings['recaptcha_secret_key'] ); if ( $recaptcha_enabled ) { wp_enqueue_script( 'nyforms-recaptcha', 'https://www.google.com/recaptcha/api.js', array(), null, true ); }
		ob_start();
		?><form class="nyforms-form <?php echo esc_attr( $definition['css_class'] ); ?>" method="post" enctype="multipart/form-data" novalidate>
			...
			<?php if ( $recaptcha_enabled ) : ?><div class="nyforms-recaptcha<?php echo 'invisible' === $recaptcha_type ? ' nyforms-recaptcha--invisible' : ''; ?>"><div class="g-recaptcha" data-sitekey="<?php echo esc_attr( $settings['recaptcha_site_key'] ); ?>"<?php echo 'invisible' === $recaptcha_type ? ' data-size="invisible" data-callback="nyformsRecaptchaComplete"' : ''; ?>></div></div><?php endif; ?>
			<?php if ( ! empty( $errors['form'] ) ) : ?><div class="nyforms-error" role="alert"><?php echo esc_html( $errors['form'] ); ?></div><?php endif; ?>
			<?php $page_count = 0; $page_open = false; foreach ( $definition['fields'] as $field ) { if ( 'page' === $field['type'] ) { if ( $page_open ) { echo '</div></div>'; } $page_open = true; $page_count++; echo '<div class="nyforms-page-panel" data-nyforms-page="' . esc_attr( $page_count ) . '"><h3>' . esc_html( $field['label'] ) . '</h3><div class="nyforms-fields">'; continue; } if ( ! $page_open ) { $page_open = true; $page_count = 1; echo '<div class="nyforms-page-panel" data-nyforms-page="1"><div class="nyforms-fields">'; } $this->field( $field, $errors['fields'][ $field['key'] ] ?? '', $resume_values[ $field['key'] ] ?? null ); } if ( $page_open ) { echo '</div></div>'; } ?>
	...
	private function field( $field, $error, $saved_value = null ) {
		...
		if ( in_array( $field['type'], array( 'html','section','page' ), true ) ) { echo '<section class="nyforms-' . esc_attr( $field['type'] ) . ' nyforms-field--width-' . esc_attr( $size ) . '">'; echo 'html' === $field['type'] ? wp_kses_post( $field['description'] ) : '<h3>' . esc_html( $field['label'] ) . '</h3>'; echo '</section>'; return; }
		if ( in_array( $field['type'], array( 'total', 'calculation' ), true ) ) { echo '<div class="nyforms-field nyforms-field--width-' . esc_attr( $size ) . ' nyforms-' . esc_attr( $field['type'] ) . '" data-nyforms-formula="' . esc_attr( $field['formula'] ) . '"><strong>' . esc_html( $field['label'] ) . '</strong><output data-nyforms-result>0.00</output></div>'; return; }
