...
class Schema {
	...
	public static function sanitize_form( $form ) {
		...
		if ( '' === $clean['title'] ) {
			return new \WP_Error( 'nyforms_invalid_title', __( 'A form title is required.', 'nyforms' ) ); }
		$keys = array();
		foreach ( (array) ( $form['fields'] ?? array() ) as $field ) {
			...
			if ( isset( $keys[ $field['key'] ] ) ) {
				return new \WP_Error( 'nyforms_duplicate_key', __( 'Every field key must be unique.', 'nyforms' ) ); }
			$keys[ $field['key'] ] = true;
			$clean['fields'][]     = $field;
		}
		$references = array_keys( $keys );
		foreach ( array_merge( $clean['fields'], $clean['confirmations'], $clean['notifications'] ) as $item ) {
			if ( ! empty( $item['conditions'] ) && ! Conditions::valid( $item['conditions'], $references ) ) {
				return new \WP_Error( 'nyforms_invalid_conditions', __( 'A conditional rule references an unavailable field.', 'nyforms' ) ); }
		}
		return $clean;
	}
	...
	private static function sanitize_settings( $settings ) {
		return array(
			...
			'progress'       => ! empty( $settings['progress'] ),
			'save_resume'    => ! empty( $settings['save_resume'] ),
	...
	private static function sanitize_confirmations( $items ) {
		...
		foreach ( (array) $items as $item ) {
			$type  = in_array( $item['type'] ?? 'message', array( 'message', 'page', 'url' ), true ) ? $item['type'] : 'message';
			$value = 'message' === $type ? wp_kses_post( $item['value'] ?? '' ) : ( 'page' === $type ? (string) absint( $item['value'] ?? 0 ) : esc_url_raw( $item['value'] ?? '' ) );
			...
			$out[] = array(
				...
				'type'       => $type,
				'value'      => $value,
				'conditions' => Conditions::sanitize( $item['conditions'] ?? array() ),
			);
		}
		...
		return $out ?: array(
			array(
				...
				'enabled'    => true,
				'type'       => 'message',
				'value'      => __( 'Thank you. Your submission has been received.', 'nyforms' ),
				'conditions' => array(),
			),
		);
	}
	...
	private static function sanitize_notifications( $items ) {
		...
		foreach ( (array) $items as $item ) {
			$out[] = array(
				'id'         => self::item_id( $item ),
				'enabled'    => ! array_key_exists( 'enabled', $item ) || ! empty( $item['enabled'] ),
				'provider'   => sanitize_key( $item['provider'] ?? 'email' ) ?: 'email',
				'to'         => sanitize_text_field( $item['to'] ?? '' ),
				'subject'    => sanitize_text_field( $item['subject'] ?? '' ),
				'message'    => wp_kses_post( $item['message'] ?? '' ),
				'from_name'  => sanitize_text_field( $item['from_name'] ?? '' ),
				'from_email' => sanitize_email( $item['from_email'] ?? '' ),
				'reply_to'   => sanitize_text_field( $item['reply_to'] ?? '' ),
				'conditions' => Conditions::sanitize( $item['conditions'] ?? array() ),
			);
		}
	...
	private static function item_id( $item ) {
