( function() {
	...
	function paginate( form ) {
		...
		var current = 0; var previous = nav.querySelector( '.nyforms-previous' ); var next = nav.querySelector( '.nyforms-next' ); var submit = nav.querySelector( '.nyforms-submit-wrap' ); var progress = nav.querySelector( '.nyforms-progress' );
			function show( index ) { current = index; pages.forEach( function( page, position ) { page.hidden = position !== current; } ); previous.hidden = current === 0; next.hidden = current === pages.length - 1; submit.hidden = current !== pages.length - 1; if ( progress ) { progress.textContent = ( window.nyformsFrontend.step || 'Step' ) + ' ' + ( current + 1 ) + ' ' + ( window.nyformsFrontend.of || 'of' ) + ' ' + pages.length; } window.scrollTo( { top: form.getBoundingClientRect().top + window.pageYOffset - 24, behavior: 'smooth' } ); }
		...
		document.addEventListener( 'DOMContentLoaded', function() { window.nyformsFrontend = window.nyformsFrontend || {}; document.querySelectorAll( '.nyforms-form' ).forEach( function( form ) { form.addEventListener( 'change', function() { update( form ); } ); var invisible = form.querySelector( '.nyforms-recaptcha--invisible' ); if ( invisible ) { form.addEventListener( 'submit', function( event ) { if ( '1' === form.dataset.nyformsRecaptchaWaiting || ! window.grecaptcha || ! window.grecaptcha.execute ) { return; } event.preventDefault(); form.dataset.nyformsRecaptchaWaiting = '1'; window.grecaptcha.execute(); } ); } update( form ); paginate( form ); } ); document.querySelectorAll( '.nyforms-privacy-request' ).forEach( function( form ) { form.addEventListener( 'submit', function( event ) { event.preventDefault(); var status = form.querySelector( '.nyforms-privacy-status' ); var token = form.querySelector( '[name="g-recaptcha-response"]' ); fetch( form.dataset.endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify( { email: form.elements.email.value, type: form.elements.type.value, 'g-recaptcha-response': token ? token.value : '' } ) } ).then( function() { status.textContent = window.nyformsFrontend.privacyRequest || ''; } ).catch( function() { status.textContent = window.nyformsFrontend.privacyRequest || ''; } ); } ); } ); } );
}() );
